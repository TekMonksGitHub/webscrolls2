/**
 * Login for Webscrolls web admin. Needs Tekmonks Unified Login to work.
 * 
 * Operations are
...
const conf = require(`${WEBSCROLLS_CONSTANTS.CONF_DIR}/webscrolls.json`);
...
async function _verifyJWT(jsonReq) {
    ...
    try {
        ...
        LOG.error(`Bad JWT token passwed for login ${jsonReq.jwt}, JWT validation succeeded but JWT claims decode failed. Error is ${err}`);
        return CONSTANTS.FALSE_RESULT;
    }
}
